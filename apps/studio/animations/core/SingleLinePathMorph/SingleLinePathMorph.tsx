 ...
 * 
 * Technical Breakdown:
 * - Single continuous SVG path that morphs between shapes
 * - Stroke-based rendering (no fill) for line art effect
 * - Animated stroke-dasharray for drawing effect
 * - Glowing stroke with shadow effects
 * - Smooth path interpolation using Framer Motion
 * 
 * SVG Path Plan:
 * - Single path that connects I → Heart → U in one continuous stroke
 * - Uses bezier curves for smooth transitions
 * - Path designed to be drawn without lifting pen
 * 
 * Framer Motion Plan:
 * - Animate path `d` attribute through morphing sequence
 * - Animate stroke-dasharray for drawing effect
 * - Animate stroke color for neon glow
 * - 5-second cycle for complete morph
 */
