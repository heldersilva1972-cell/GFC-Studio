...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        {
            var options = sp.GetRequiredService<IOptions<AccessControlSimulationOptions>>().Value;
            return options.UseAccessControlSimulation
        ...
        // Note: This resolution is lazy - it only happens when IMengqiControllerClient is requested,
        // not during service registration, so database access is safe.
        builder.Services.AddScoped<IMengqiControllerClient>(sp =>
        {
            try
            {
                ...
                {
                    return sp.GetRequiredService<GFC.BlazorServer.Services.SimulationControllerClient>();
                }
                else
                {
                    return sp.GetRequiredService<GFC.BlazorServer.Services.RealControllerClient>();
                }
            }
            catch (Exception ex)
            {
                // If there's any error accessing the mode provider (e.g., database not ready),
