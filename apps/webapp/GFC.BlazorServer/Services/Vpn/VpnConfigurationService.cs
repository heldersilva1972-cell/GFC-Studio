...
public class VpnConfigurationService : IVpnConfigurationService
{
    ...
    public async Task<string> CreateOnboardingTokenAsync(int userId, int durationHours = 48, bool isStation = false)
    {
        using var context = await _dbContextFactory.CreateDbContextAsync();
    ...
    public async Task<OnboardingTokenResult?> ValidateOnboardingTokenAsync(string token)
    {
        ...
        return new OnboardingTokenResult(record.UserId, record.IsStation);
    }
