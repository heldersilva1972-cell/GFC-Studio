-- ========================================================
-- Migration: Recategorize Bingo & Restore Master Sidebar Pages
-- Purpose: Ensures all 76 sidebar and terminal pages are 
--          fully seeded, categorized, and active in the database.
-- ========================================================
USE ClubMembership;

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'AppPages')
BEGIN
    PRINT 'Starting Master Sidebar Restoration and Page Recategorization...';

    -- 1. Helper Table for all 76 pages
    DECLARE @MasterPages TABLE (
        Name NVARCHAR(100),
        Route NVARCHAR(255),
    ...
    INSERT INTO @MasterPages (Name, Route, Category, Descr, IsAdmin, DisplayOrder) VALUES
    -- GENERAL
    ...
    -- 2. Sync the temporary list to the real AppPages table
    MERGE AppPages AS target
    USING @MasterPages AS source
    ...
    -- 3. Cleanup any old orphaned pages
    DELETE FROM AppPages WHERE IsActive = 0;

    PRINT 'Master Sidebar Restoration completed successfully. All 76 pages are active.';
END
ELSE
BEGIN
    PRINT 'Error: AppPages table does not exist in the current database context.';
END
GO
