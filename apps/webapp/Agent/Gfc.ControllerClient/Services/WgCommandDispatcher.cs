...
internal sealed class WgCommandDispatcher
{
    ...
    public async Task<byte[]> SendAsync(uint controllerSn, WgCommandProfile profile, ReadOnlySpan<byte> payload, CancellationToken cancellationToken)
    {
        var endpoint = _endpointResolver.Resolve(controllerSn);
        var request = _packetBuilder.Build(controllerSn, NextXid(), profile, payload);
