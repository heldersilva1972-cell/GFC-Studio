...
public class MobileDeviceTrustService : IDeviceTrustService
{
    ...
    public async Task<bool> ValidateTokenAsync(string token)
    {
        ...
        try {
            var response = await _http.GetFromJsonAsync<DeviceValidationResponse>($"/api/mobile-auth/validate-device?token={token}");
    ...
    private class SetupCodeRedemptionResponse
    {
    ...
    public async Task<string?> ValidateSetupCodeAsync(string code)
    {
        ...
        try {
            var response = await _http.PostAsJsonAsync("/api/mobile-auth/redeem-setup-code", new { Code = code });
            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadFromJsonAsync<SetupCodeRedemptionResponse>();
                return result?.Token;
            }
            return null;
        } catch { return null; }
