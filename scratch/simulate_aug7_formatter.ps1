...
# Now let's simulate ZReportReceiptFormatter groupedRegular logic in PowerShell
...
Write-Host "`n--- REGULAR SALES SUMMARY KVP & PRICE LOOKUP ---"
...
foreach ($key in $regularSalesSummary.Keys) {
    ...
    if ($key.Contains("(TOKEN REDEEMED)") -or $key.Contains("TOKEN REDEEMED")) {
        ...
        if ($key.Contains("TOKEN") -or $key.Contains("Token")) {
    ...
    Write-Host ("{0,-45} | Qty: {1,2} | LookupPrice: {2,6:C2} | LineTotal: {3,7:C2} | DB Category: {4,-10} | Assigned: {5}" -f $key, $qty, $price, $lineTotal, $category, $assignedGroup)
}

Write-Host "--------------------------------------------------------------------------"
Write-Host ("Calculated FOOD & SNACKS TOTAL:   {0:C2}" -f $foodSnackTotal)
Write-Host ("Calculated LIQUOR & DRINKS TOTAL: {0:C2}" -f $liquorTotal)
